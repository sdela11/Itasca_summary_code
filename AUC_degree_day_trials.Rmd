...
num.nanas <- degree.hours[is.na(degree.hours$date.time),]
print(length(c(num.nanas$date.time))) 

# WARNING: Time-intensive calculations below!!!
...
degree.hours <- degree.hours %>%   
  ...
  summarise(site = site, 
            rep = rep,
            position = position, 
            degree.hours = sum(diff(hour(date.time))*rollmean(value,2), na.rm = TRUE)) %>% 
  distinct(site, rep, position, .keep_all = TRUE) %>% 
  ungroup()
view(degree.hours)
str(degree.hours)
write_csv(degree.hours, file = "AUC_degree_hours_prelim.csv", )
