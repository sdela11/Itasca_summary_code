...
csv_LIST <- lapply(seq_along(files.meta.list), function(i){   
  ...
  df$date.time <- fast_strptime(df$date.time, format = c("%m/%d/%Y %H:%M", "%m/%d/%Y %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%d %H:%M:%S"), lt = FALSE)
  #df$date.time <- as.POSIXct(df$date.time, format = "%Y-%m-%d %H:%M") %>% round("mins")
...
#DON'T DO THIS: finalTable$date.time <- as.POSIXct(finalTable$date.time, format = c("%m/%d/%Y %H:%M", %m/%d/%Y %H:%M:%S")) %>% round("mins") #get rid of any potential "seconds" values
...
annual.sums.df <- data %>% 
  as.POSIXct(data$date.time, format = "%m/%d/%Y %H:%M") #%>% 
