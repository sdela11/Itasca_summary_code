...
annual.sums.df.2020 <- data %>%
  group_by(site, rep, position) %>% 
  summarise(meantemp = mean(value))

view(annual.sums.df.2020)
write_csv(annual.sums.df.2020, "annual_sums_2020_1.csv")
...
```{r}
#Annual summaries for October 2020 - September 2021
...
annual.sums.df.SEP <- data %>%
  group_by(site, rep, position) %>% 
  summarise(meantemp = mean(value), length = length(date.time)) #how many values are in each group? Use this column to weed out sensors with only a few cells of data.
annual.sums.df.SEP <- annual.sums.df.SEP %>% 
  subset.data.frame(length > 500) #only select rows where the length column value is greater than 500.

view(annual.sums.df.SEP)
write_csv(annual.sums.df.SEP, "annual_sums_OCT_1.csv")
```
...
myplot <- ggplot(plot.data.z, aes(x = site, y = mean(value), color = position)) + 
