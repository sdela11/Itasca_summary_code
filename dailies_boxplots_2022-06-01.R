...
input.df <- tibble(month = sprintf("%02d", month(month.seq)), year = year(month.seq)) #create tibble out of separate elements, use sprintf to include leading zero.
view(input.df)
map2(input.df$month, input.df$year, Boxplot.FUN)



test.string <- as.POSIXct(glue("{data[1,2]}-{data[1,1]}-01"))
